...
class Profiler {
  ...
  public:
    Profiler(int timed_runs = 10, int warmup_runs = 2) : timed_runs_(timed_runs), warmup_runs_(warmup_runs) {}
    void add(const std::string &name, std::function<void()> launch, std::function<bool()> validate = nullptr,
             size_t bytes = 0, size_t flops = 0) {
...
template <typename T> class Randomizer {
  public:
    ...
    Randomizer(T min, T max, unsigned int seed = 43) : gen(seed) { dist = std::uniform_real_distribution<T>(min, max); }
